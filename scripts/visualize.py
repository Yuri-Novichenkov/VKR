...
def _resolve_predictions_file(predictions_file):
    if predictions_file and os.path.exists(predictions_file):
        return predictions_file
    # Попытка найти файл предсказаний по умолчанию
    for candidate in ("predictions_pointnetpp.txt", "predictions.txt", "predictions_pointnet.txt"):
        if os.path.exists(candidate):
            return candidate
    return None


def visualize_point_cloud_sample(data_file, predictions_file=None, num_points=10000, save_dir='.', color_by='class'):
    ...
    # Нормализация координат для лучшей визуализации (только для отображения)
    x = (x - x.mean()) / x.std()
    y = (y - y.mean()) / y.std()
    z = (z - z.mean()) / z.std()
    ...
    # 1) Если есть файл с предсказаниями — используем его
    predictions_path = _resolve_predictions_file(predictions_file)
    if predictions_path:
        pred_data = pd.read_csv(predictions_path, sep='\t', nrows=num_points)
        if 'Predicted_Classification' in pred_data.columns:
            colors = pred_data['Predicted_Classification'].values[:num_points]
            title_suffix = f' (с предсказаниями: {os.path.basename(predictions_path)})'
    
    # 2) Если нет предсказаний, но есть ground truth
    if colors is None and 'Classification' in data.columns:
        colors = data['Classification'].values[:num_points]
        title_suffix = ' (ground truth)'
    
    # 3) Если все еще нет классов — используем высоту только при явном запросе
    if colors is None:
        if color_by == 'height':
            colors = z
            title_suffix = ' (цвет = высота Z)'
        else:
            raise ValueError(
                "Не найдены классы для визуализации. "
                "Передайте --predictions с файлом Predicted_Classification "
                "или используйте --color_by height."
            )
...
def main():
    ...
    parser.add_argument('--color_by', type=str, default='class', choices=['class', 'height'],
                       help='Как раскрашивать точки: class (по классам) или height (по высоте Z)')
    ...
    if args.test_data:
        print("\n5. Визуализация облака точек...")
        visualize_point_cloud_sample(
            args.test_data,
            args.predictions,
            num_points=1_400_000,
